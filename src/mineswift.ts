...
class Minefield extends Array<Array<Cell>>
{
    ...
    * @throws If the cell position is invalid.
    */
   isSolvableFrom([row, col]: Position, restore: boolean=true): boolean
   {
   ...
   #validatePosition(...position: Position | [Position]): Position
   {
      ...
      try
      {
         ...
         if (isNaN(row) || isNaN(col)) throw 0;
      }
      catch
      {
   ...
   {
      position = ([] as number[]).concat(...position) as (number[] | Position)

      if (position.length == 1) position = this.#indexToPosition(position[0])
    ...
    */
   visualize({ unicode=false, positions=false, color=false, highlight=[], uncover=false, log=true }: { unicode?: boolean, positions?: boolean, color?: boolean, highlight?: Position[], uncover?: boolean, log?: boolean } = {}): string
   {
